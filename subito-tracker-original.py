...
def run_query(url, name, notify, minPrice, maxPrice):
    ...
    for product in product_list_items:
        ...
        sold = product.find('span', class_=re.compile(r'sold'))
        if sold is not None:
            continue
        try:
        ...
        try:
            location = product.find('span', re.compile(r'town')).string + product.find('span',
                                                                                       re.compile(r'city')).string
        except:
            ...
            if maxPrice == "null" or price == "Unknown price" or price[:-2] <= maxPrice:
                if not queries.get(name):  # insert the new search
                    ...
                    if not queries.get(name).get(url).get(minPrice).get(maxPrice).get(link):  # found a new element
                        ...
                        msg.append(tmp)
                        queries[name][url][minPrice][maxPrice][link] = {'title': title, 'price': price,
                                                                        'location': location}
